/* C++ version of the WCCd dissimilarity, originally written in C for the wccsom package*/
#include <Rcpp.h>
...
double WCCd3(double *p1, double *p2, int np, int nNA) {
  int i;
  double crosscov, wght, acor1, acor2, trwdth = 3.0;
...
double wcc_autocorr(double *p1, int np) {
  int i;
  double autocov, wght, trwdth = 3.0;
...
/* If you want to use wccd3 inside R, you can call the function with this
   applyDF wrapper:
   wccvalue <- applyDF(wccd3, pattern1, pattern2, numNA) */
...
// [[Rcpp::export]]
Rcpp::XPtr<DistanceFunctionPtr> WCCd3() {
  return (Rcpp::XPtr<DistanceFunctionPtr>(new DistanceFunctionPtr(&WCCd3)));
}
