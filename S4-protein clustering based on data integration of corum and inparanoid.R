...
setMethod("ProteinComplexGoldStandardPredictionWithinCORUMOrthologousComplexes", signature("Protein.Complexes.Gold.Standard.Prediction"),
          function(Protein.Complexes.Gold.Standard.Prediction){
            ...
            subcomplex_goldstd <- mutate(subcomplex_goldstd, 
                                         Ratio_Mapp_Mcalc_subcomplex=abs(as.numeric(Mapp_Subcomplex)-as.numeric(Mcalc_Subcomplex))/as.numeric(Mcalc_Subcomplex))
            ...
            data_ratiosmall_Rapplarge_nosingleton <- filter(subcomplex_goldstd_Rapplarge_nosingleton,Ratio_Mapp_Mcalc_subcomplex<=1)
            ...
            cat("\tStep 3.6 has filtered out", length(com_sub_ratiosmall_Rapplarge_nosingleton),  "non-duplicate subcomplexes with p-value<=0.05 and Mapp_Mcalc_Ratio<=1 without monomer.\n",
            ...
            for (i in 1:dim(gold_std_result_summary)[1]){
              if (gold_std_result_summary[i,"Ratio_Mapp_Mcalc_subcomplex"]<=1){
                gold_std_result_summary[i,"class_ratio_Mapp_Mcalc"] <- "Ratio <= 1"
              }else{
                gold_std_result_summary[i,"class_ratio_Mapp_Mcalc"] <- "Ratio > 1"
              }
            }
            ...
            if (length(fullyAssembledComplexes)>0){
              ...
              for (i in 1:dim(subcomplex_goldstd_Rapplarge_nosingleton)[1]){
                if (subcomplex_goldstd_Rapplarge_nosingleton[i,"Ratio_Mapp_Mcalc_subcomplex"]<=1){
                  subcomplex_goldstd_Rapplarge_nosingleton[i,"Class"] <- "Ratio<=1"
                }else{
                  subcomplex_goldstd_Rapplarge_nosingleton[i,"Class"] <- "Ratio>1"
                }
              }
